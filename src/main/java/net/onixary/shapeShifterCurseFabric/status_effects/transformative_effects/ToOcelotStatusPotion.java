...
public class ToOcelotStatusPotion extends StatusEffect {
    public ToOcelotStatusPotion() {
    ...
    public void applyInstantEffect(@Nullable Entity source, @Nullable Entity attacker, LivingEntity target, int amplifier, double proximity)
    {
        if (source instanceof ServerPlayerEntity player) {
            ...
            if (curToForm != TO_OCELOT_0_EFFECT.getToForm() && FormAbilityManager.getForm(player) == PlayerForms.ORIGINAL_SHIFTER){
                EffectManager.overrideEffect(player, TO_OCELOT_0_EFFECT);
            }
        }
    }
