...
public class ToFeralCatStatusPotion extends StatusEffect {
    public ToFeralCatStatusPotion() {
    ...
    public void applyInstantEffect(@Nullable Entity source, @Nullable Entity attacker, LivingEntity target, int amplifier, double proximity)
    {
        if (!target.getWorld().isClient() && target instanceof ServerPlayerEntity player) {
            ...
            if (curToForm != TO_FERAL_CAT_SP_EFFECT.getToForm() && FormAbilityManager.getForm(player) == PlayerForms.ORIGINAL_SHIFTER){
                EffectManager.overrideEffect(player, TO_FERAL_CAT_SP_EFFECT);
            }
        }
    }
