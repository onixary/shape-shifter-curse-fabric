...
public class ToAlphaStatusPotion extends StatusEffect {
    public ToAlphaStatusPotion() {
    ...
    public void applyInstantEffect(@Nullable Entity source, @Nullable Entity attacker, LivingEntity target, int amplifier, double proximity)
    {
        if (!target.getWorld().isClient() && target instanceof ServerPlayerEntity player) {
            ...
            if (curToForm != TO_ALPHA_0_EFFECT.getToForm() && FormAbilityManager.getForm(player) == PlayerForms.ORIGINAL_SHIFTER){
                EffectManager.overrideEffect(player, TO_ALPHA_0_EFFECT);
            }
        }
    }
