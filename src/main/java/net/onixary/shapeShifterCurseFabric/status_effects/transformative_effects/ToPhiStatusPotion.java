...
import static net.onixary.shapeShifterCurseFabric.status_effects.RegTStatusEffect.TO_PHI_SP_EFFECT;
...
public class ToPhiStatusPotion extends StatusEffect {
    public ToPhiStatusPotion() {
    ...
    public void applyInstantEffect(@Nullable Entity source, @Nullable Entity attacker, LivingEntity target, int amplifier, double proximity)
    {
        if (!target.getWorld().isClient() && target instanceof ServerPlayerEntity player) {
            ...
            if (curToForm != TO_PHI_SP_EFFECT.getToForm() && FormAbilityManager.getForm(player) == PlayerForms.ORIGINAL_SHIFTER){
                EffectManager.overrideEffect(player, TO_PHI_SP_EFFECT);
            }
        }
    }
